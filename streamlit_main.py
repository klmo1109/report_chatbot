...
PASSWORD = os.getenv("STREAMLIT_APP_PASSWORD")
...
if not st.session_state.access_granted:
    password_container = st.empty()  # 創建可動態更新的容器
    message_container = st.empty()   # 創建顯示訊息的容器

    password = password_container.text_input("輸入密碼即可使用:", type="password")

    if password and password == PASSWORD:
        st.session_state.access_granted = True
        message_container.success("✅ 密碼正確，功能已解鎖！")
        time.sleep(1.5)  # 短暫顯示後清除
        password_container.empty()  # 清除密碼輸入框
        message_container.empty()  # 清除成功訊息
    elif password:
        st.session_state.access_granted = False
        message_container.error("❌ 密碼錯誤，請重新輸入！")

# **限制功能**
if not st.session_state.access_granted:
    st.warning("⚠️ 請輸入正確密碼後才能使用其他功能！")
    st.stop()  # 停止執行後續的 Streamlit 代碼
